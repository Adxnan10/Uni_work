...
public class BST<T extends Comparable<? super T>> {
    ...
    // My methods
    public int count() {
    	return count(root) - 1; // since the overloaded count will add up 2 null elements and will not add the root, the method will return the size + 1. So, I just subtracted 1. 
    }
    ...
    protected int countLeaves(BSTNode<T> p) {
    	if (p.left == null && p.right == null)
    		return 1;
    	else if (p.left == null)
    		return countLeaves(p.right);
    	else if (p.right == null)
    		return countLeaves(p.left);
    	else 
    ...
    protected int height(BSTNode<T> p) {
    	if (p == null)
    		return 0;
