...
public class BST<T extends Comparable<? super T>> {
    ...
    public T shallowestLeaf() {
        return shallowestLeafRecursive(root, minPath(), 0);
    }

    public T deepestLeaf() {
    	int height = height(root) - 1;
        return deepestLeafRecursive(root, height, 0);
    }

    public T shallowestLeafRecursive(BSTNode<T> node, int path, int level) {
    	if(node == null)
    		return null;
    	if (level == path) {
    		...
    		T left = shallowestLeafRecursive(node.left, path, level + 1);
    		T right = shallowestLeafRecursive(node.right, path, level + 1);
    		if (left == null && right == null)
    			...
    			return left;
    		return left.compareTo(right) < 1 ? left : right; // if two leaves were found in the same level, choose the minimum value
    	}
		return null; // Exceeded shallowest path -> No need to go further
    	
    }

    public T deepestLeafRecursive(BSTNode<T> node, int height, int level) {
    	if(node == null)
    		return null;
    	if (level == height) {
    		if (node.left == null && node.right == null )
    		...
    		T left = deepestLeafRecursive(node.left, height, level + 1);
    		T right = deepestLeafRecursive(node.right, height, level + 1);
    		if (left == null && right == null)
    			return null;
    		else if (left == null)
    			return right;
    			...
    			return left;
    		return left.compareTo(right) > 1 ? left : right; // if two leaves were found in the same level, choose the maximum value
    	}
		return null; // Just in case level exceeded height, although it should never happen.
    }

    protected int height(BSTNode<T> p) { // going to use this method to find the deepest node 
    	if (p == null)
    		return 0;
    	return 1 + Math.max(height(p.left), height(p.right));
    }
    public int minPath() { // going to use this method to find the shallowest node 
    	if (root == null) // handling empty tree
    		return 0;
		return minPath(root);
    }
    protected int minPath(BSTNode<T> node) {
    	if (node.left == null && node.right == null) // leaf case
    		return 0;
    	else if (node.left == null)
    		return 1 + minPath(node.right);
    	else if (node.right == null)
    		return 1 + minPath(node.left);
    	
		return 1 + Math.min(minPath(node.left), minPath(node.right)); 
    }
    public void clear() {
        root = null;
    }
