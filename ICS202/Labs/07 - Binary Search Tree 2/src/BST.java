...
public class BST<T extends Comparable<? super T>> {
    ...
    public String getPath(T t1) {
    	return getPath(t1, root);
    }
    protected String getPath(T t1, BSTNode<T> p) {
    	if (p == null) 
    		return " PATH NOT FOUND";
    	else 
    		if (t1.compareTo(p.el) < 0) 
    			return p.el + " " +  getPath(t1, p.left);
    		else if (t1.compareTo(p.el) > 0)
    			return p.el + " " + getPath(t1, p.right);
    		else 
    			return p.el + " ";
    }
    public int getRightLeafCount() {
    	return getRightLeafCount(false, root);
    }
    protected int getRightLeafCount(boolean isRight, BSTNode<T> p) {
    	if (p.left == null && p.right == null)
    		return isRight? 1: 0;
    	else if (p.left == null)
    		return getRightLeafCount(true, p.right);
    	else if (p.right == null)
    		return getRightLeafCount(false, p.left);
    	else 
    		return getRightLeafCount(false, p.left) + getRightLeafCount(true, p.right);
    }
    public int rangeCounter(T t1, T t2) {
    	return rangeCounter(t1, t2, root);
    }
    protected int rangeCounter(T t1, T t2, BSTNode<T> p) {
    	if (p == null) return 0;
    	if (t2.compareTo(p.el) > 0 && t1.compareTo(p.el) < 0)
    		return 1 + rangeCounter(t1, t2, p.left) + rangeCounter(t1, t2, p.right);
    	else 
    		return 0 + rangeCounter(t1, t2, p.left) + rangeCounter(t1, t2, p.right); 
    }
